...
@interface LXF_YLImageView ()

@property (nonatomic, strong) LXF_YLGIFImage *animatedImage;
...
@implementation LXF_YLImageView
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([image isKindOfClass:[LXF_YLGIFImage class]] && image.images) {
        if([image.images[0] isKindOfClass:UIImage.class])
            [super setImage:image.images[0]];
        else
            [super setImage:nil];
        self.currentFrame = nil;
        self.animatedImage = (LXF_YLGIFImage *)image;
...
- (void)setAnimatedImage:(LXF_YLGIFImage *)animatedImage
{
